			...
			<div class="col-md-12">
				<div class="card  border-primary">
					<div class="card-header text-center text-white bg-primary">
						<h5>Solicitudes Centro de Costos(Firmas Anexas)</h5>
					</div>
						...
						<div class="table-responsive">
							<div v-cloak class="mt-4">
								<v-client-table :columns="columns" v-model="data" :options="options">
									...
									<a slot="prioridad" slot-scope="props">
										<a href="#"  v-if="props.row.prioridad == 1"><img :src="'assets/img/priority.png'" width="40" height="30"></a>
									</a>
									<a slot="accion" slot-scope="props">


										<button class="btn btn-success" @click="visualizarnuevo(props.row.id_doc,props.row.tipo,props.row.cc,props.row.id,props.row.prioridad)">Visualizar</button>
	...
	export default {
		...
		data(){
			...
			return {
				...
				id_categoria:'',
				categoria:'',
				columns: ['id','cc_nombre', 'compra_numero', 'compra_amio','fondo','prioridad','accion'],
				data: [],
				options: {
					perPage: 15,
					texts: {
          ...
          if(tipo == 1){
          	...
          	axios.post(url,d).then(response=>{
          		...
          		axios.post(url2,da).then(resp=>{
                    ...
                    this.crearhistoria(this.idcomprar,5,6);
                    this.crearhistoria2(this.idcomprar,5,1)
          			this.getCentroCosto();
          	...
          	this.updatedsolicitud(this.id_so,this.prioridad,this.numero,this.idcomprar,this.cc_id);
          	this.addfirmassolicitud(this.id_so,this.prioridad);
          	$("#modalaceptar").modal('hide');
          	this.getCentroCosto();
			...
			crearhistoria2(id,estado,actividad){
			...
			crearhistoria(id,estado,actividad){
				...
				var url="historialdecompras2/"+id;
				var d={
					actividad:1,				
					usuario:this.id_session,
					_method:'patch'
				}
				axios.post('historialdecompras2',d).then(response=>{
			...
			updatedsolicitud(id,prioridad,numero,id_doc,cc_id)
			{
				var url="solicitudes/"+id
				var estado
				if(prioridad == 0){
					...
					axios.post(url,d).then(response=>{
						this.addsolicitudes(numero,id_doc,cc_id);
					}).catch(err=>{
			...
			visualizarnuevo(id,tipo,cc,id_s,prioridad)
			{ 
				this.prioridad=''	
...
getordenes(id){
	axios.get('ordenesdecompra/'+id).then(response=>{
		this.cc_id=response.data[0].cc_id
...
getViaticos(id){
	...
	axios.post('viaticos_jt',d).then(response=>{
      ...
      //this.getProductos(response.data[0].compra_id);
      this.buscarprespuestos(response.data[0].v_cc,22);
      $("#modalaceptar").modal();
      this.bloqueo=false
  }).catch(eer=>{
  	this.bloqueo=false
...
getCentroCosto(){
	...
	const dia = fecha.getDate();
          ...
          d.append('tipo',5);
          this.bloqueo=true;
          axios.post('solicitudes4',d).then(response=>{
