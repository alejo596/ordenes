...
<script>
 export default {
  ...
  {
   Swal.fire({
    title: 'Desea rechazar solicitud?',          
    icon: 'warning',
    input:'textarea',     
...
rechazarot(obs){
  var url="../ordenesdecompra"+this.idcomprar
  var d={
    compra_estado:11,
...
axios.post('../firmasolicitudes',d).then(response=>{
 $("#modal_fondo").modal('hide');
        ...
        const dia = fecha.getDate();
          ...
          axios.post(url,d).then(response=>{
            ...
            axios.post(url2,da).then(resp=>{
              ...
              this.verificarsolicitudes(this.idcomprar,this.id_so,this.numero,this.cc_id)
              this.crearhistoria(this.idcomprar,3,1);
              this.getCentroCosto();
  ...
  verificarsolicitudes(id,solicitud,numero,cc){
   axios.get('../solicitudes/'+id).then(response=>{
         if(response.data === 0){
          console.log("verificacion1")
          
             this.updatedsolicitud(solicitud);
             this.addsolicitudes(numero,id,cc);
             this.addfirmassolicitud(this.id_so,9);
            $("#modal_fondo").modal('hide');
          this.getCentroCosto();
         }else{
...
getFirmas(id)
{
  axios.get('../firmaot/'+id).then(response=>{
    ...
    for (var i = 0; i < this.firmas.length; i++) {
      //this.firmas[i]
    }
  }).catch(err=>{
    this.bloqueo=false;
...
getViaticos(id){
 ...
 this.year=response.data[0].v_year;ssss
